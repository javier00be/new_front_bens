...
import {
    FileText, Tag, Layers, Ruler, CreditCard, Receipt,
...
type TabKey = 'tipo-documento' | 'categorias' | 'marcas' | 'tallas' | 'medios-pago' | 'tipo-comprobante';
...
// ─── Sección TipoComprobante (nombre + abreviatura) ──────────────────────────

const TipoComprobanteSection = () => {
    const [items,         setItems]         = useState<TipoDocItem[]>([]);
    const [isLoading,     setIsLoading]     = useState(true);
    const [isAdding,      setIsAdding]      = useState(false);
    const [newNombre,     setNewNombre]     = useState("");
    ...
    useEffect(() => {
        api.get<TipoDocItem[]>("/tipo-comprobante")
            .then(r => setItems(r.data))
            .catch(() => {})
            .finally(() => setIsLoading(false));
    }, []);
    ...
    const handleAdd = async () => {
        ...
        setIsSaving(true);
        try {
            const r = await api.post<TipoDocItem>("/tipo-comprobante", {
                nombre: newNombre.trim(),
                abreviatura: newAbrev.trim().toUpperCase(),
            });
            setItems(prev => [...prev, r.data]);
            setNewNombre(""); setNewAbrev(""); setIsAdding(false);
            sileo.success({ title: "Comprobante creado", description: `"${r.data.abreviatura}" fue agregado.` });
        } catch {
            sileo.error({ title: "Error", description: "No se pudo crear. Verifica que no exista ya." });
        } finally {
            setIsSaving(false);
    ...
    const handleEdit = async (id: number) => {
        ...
        setIsSaving(true);
        try {
            const r = await api.patch<TipoDocItem>(`/tipo-comprobante/${id}`, {
                nombre: editNombre.trim(),
                abreviatura: editAbrev.trim().toUpperCase(),
            });
            setItems(prev => prev.map(i => i.id === id ? r.data : i));
            setEditingId(null);
            sileo.success({ title: "Comprobante actualizado", description: `"${r.data.abreviatura}" guardado.` });
        } catch {
            sileo.error({ title: "Error", description: "No se pudo actualizar." });
        } finally {
            setIsSaving(false);
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await api.delete(`/tipo-comprobante/${confirmDelete.id}`);
            setItems(prev => prev.filter(i => i.id !== confirmDelete.id));
            sileo.success({ title: "Comprobante eliminado", description: `"${confirmDelete.abreviatura}" fue eliminado.` });
        } catch {
            sileo.error({ title: "Error", description: "No se pudo eliminar. Puede estar en uso por un pedido." });
        } finally {
            setIsDeleting(false);
            setConfirmDelete(null);
        }
    ...
    return (
        <div className="space-y-3">
            {isAdding ? (
                ...
                <Button size="sm" variant="outline" onClick={() => setIsAdding(true)} className="text-xs h-8 border-dashed">
                    <Plus className="w-3.5 h-3.5 mr-1" /> Agregar tipo de comprobante
                </Button>
            )}
            ...
            <ConfirmModal
                ...
                isLoading={isDeleting}
                onConfirm={handleDelete}
                description={<>¿Eliminar <strong className="text-slate-800">"{confirmDelete?.abreviatura}"</strong>? Puede fallar si está en uso por un pedido.</>}
            />
        </div>
    );
};
...
const TABS: { key: TabKey; label: string; icon: React.ReactNode; description: string }[] = [
    ...
    { key: "tipo-comprobante",  label: "Tipos de Comprobante", icon: <Receipt className="w-4 h-4" />,    description: "Boleta, Factura, Ticket..." },
];
                ...
                <div className="flex-1 min-w-0">
                    <Card className="border border-slate-100 shadow-sm rounded-2xl">
                        <CardContent className="p-6">
                            ...
                            {activeTab === "tipo-comprobante" && <TipoComprobanteSection />}
                        </CardContent>
                    </Card>
                </div>
            </div>
